...
// ለሁሉም ተጫዋቾች ወቅታዊ የጨዋታ ሁኔታ መላኪያ function
function broadcastRoomState() {
    const soldCardsCount = Object.keys(roomState.cardOwners).length;
    const derashAmount = soldCardsCount * 10; // ደራሽ = የካርቴላዎች ብዛት × 10 ብር

    io.emit('roomState', {
        soldCards: Object.keys(roomState.cardOwners).map(Number),
        timeLeft: roomState.timeLeft,
        playersCount: soldCardsCount, // የካርቴላዎች ብዛት እንደ Players Count
        derash: derashAmount,         // አጠቃላይ የተሰበሰበው ደራሽ መጠን
...
    
    // ተጫዋቹ እንደገባ የተጫዋቹን መረጃ እና ቀሪ ሂሳብ ከዳታቤዝ አምጥቶ መላክ
...
            socket.emit('userBalanceUpdate', { balance: updatedUser.balance });
            broadcastRoomState();
        }
    });
